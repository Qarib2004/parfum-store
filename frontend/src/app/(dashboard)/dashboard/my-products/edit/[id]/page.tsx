...
export default function EditProductPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (product && user && product.ownerId !== user.id && user.role !== 'ADMIN') {
  ...
  if (!user || (user.role !== 'OWNER' && user.role !== 'ADMIN')) {
    return (
      <div className={style.statePage}>
        <p className={style.stateText}>You don't have permission to edit products</p>
      </div>
    );
  }
  ...
  if (product.ownerId !== user.id && user.role !== 'ADMIN') {
    return (
      <div className={style.statePage}>
        <h2 className={style.stateTitle}>You don't have permission to edit this product</h2>
        <Link href="/dashboard/my-products" className={style.stateLink}>
          Back to my products
        </Link>
      </div>
