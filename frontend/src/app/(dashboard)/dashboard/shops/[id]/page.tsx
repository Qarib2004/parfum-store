...
import { Pagination, Product, Shop } from "@/types";
...
export default function ShopDetailsPage() {
  ...
  const slug = params?.id && params.id !== "null" ? params.id : null;

  const { data: shopData, isLoading: shopLoading } = useQuery({
    queryKey: ["shop", slug],
    queryFn: async () => {
      const response = await shopApi.getShopById(slug!);
  ...
  const { data: productsData, isLoading: productsLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await shopApi.getShopProducts(slug!, {
        page: 1,
        limit: 12,
      });
        ...
        <div className={style.headerMain}>
          ...
          <div>
            ...
            <p className={style.meta}>
              Created {formatRelativeDate(shop.createdAt)}
            </p>
          </div>
        </div>
      </header>
        ...
        {products.length > 0 ? (
          ...
          <div className={style.empty}>
            ...
            <p className={style.emptyDesc}>
              This shop has no products in the catalog.
            </p>
          </div>
        )}
      </section>
